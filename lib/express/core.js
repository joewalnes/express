...
Router = new Class({
  ...
  route: function() { 
    ...
    if (route) {
      ...
      if(this.request.hasContentTypeCallbacks()) {
        var content_callback = this.request.contentCallback()
        if(content_callback) {
          body = content_callback.apply(this.request);
        } else {
          this.request.respond(415)
        }
      }
      return body
    }
